# -*- coding: utf-8 -*-
""" 
 Created on 11/15/2019 10:02 AM
...
GPGGA = re.compile("""
    ...
    # $GNGGA,020017.79,4003.8515899,N,11613.6874606,E,4,28,0.55,54.611,M,,M,1.000,0000*4E

    ^\$GNGGA,
    (?P<hhmmss>\d{6}(\.\d{2})),      # hhmmss = '035306.200'
    (?P<latitude>\d+\.\d+),          # latitude = '4735.7144'
    (?P<N_S>[NS]),                   # N_S = 'N'
    (?P<longitude>\d+\.\d+),         # longitude = '12219.6396'
    (?P<W_E>[WE]),                   # W_E = 'W'
    (?P<fix_qual>[0123456]),             # fix_qual = '1'
    (?P<num_sat>\d+),                # num_sat = '8'
    (?P<hdop>\d+(\.\d+)),            # hdop = 1.42
    (?P<altitude>\d+(\.\d+)),M, # altitude = -2.1
    (?P<height>([!\,]+)?),M,         # dgps = ''
    (?P<dgps>(\-)\d+(\.\d+))
    (?P<checksum>\d+\*\w\w)$            # checksum = '*46'
""", re.VERBOSE)
